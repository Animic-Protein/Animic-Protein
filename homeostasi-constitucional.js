(() => {
  ...
  const contextualState=branch=>{
    const rawState=pressureState(branch.id),raw=Math.max(0,Number(rawState.score)||0),old=stateFor(branch);
    const basis=Math.max(0,Number(old.pressureBasis)||0);
    const drift=Math.abs(raw-basis);
    ...
    return {rawState,raw,old,relief,stale,effective:Math.max(0,raw-relief),limit:threshold(branch),floor:interventionFloor(branch)};
  };

  const apply=(branch,mode)=>{
    const info=modeInfo[mode];if(!branch||!info)return null;
    const s=contextualState(branch);if(s.raw<s.floor)return s.old;
  ...
  const resetAfterMutation=event=>{
    ...
    const reset={id:`homeo-${Date.now().toString(36)}`,mode:'reset',label:'Mutació constitucional',amount:-(Number(old.relief)||0),rawPressure:raw,effectiveBefore:Math.max(0,raw-(Number(old.relief)||0)),effectiveAfter:raw,at:now};
  ...
  const render=()=>{
    ...
    const s=contextualState(branch),components=s.rawState.components||{};
    ...
    const conflict=Number(components.conflicts)||0,metabolism=Number(components.metabolism)||0;
    signal.textContent=s.raw<s.floor?'Encara no hi ha prou pressió per intervenir. Homeòstasi observa.':s.stale?'La pressió ha canviat prou perquè l’alleujament anterior deixi de ser vàlid. Cal tornar a escoltar el sistema.':conflict>0||metabolism>0?'La pressió conté fricció o transformació real: es pot intervenir temporalment sense ocultar-la.':'La pressió és sobretot activitat acumulada. Intervé amb prudència: activitat no és desequilibri.';box.appendChild(signal);
    const actions=document.createElement('div');actions.className='homeo-actions';
    Object.entries(modeInfo).forEach(([mode,info])=>{const button=document.createElement('button');button.type='button';button.className=`homeo-action mode-${mode}`;button.textContent=info.label;button.title=info.note;button.disabled=s.raw<s.floor||s.relief>=maxRelief(branch)||s.stale;button.addEventListener('click',()=>{const result=apply(branch,mode),out=document.getElementById('relation-output');if(out&&result)out.innerHTML=`<strong>Homeòstasi constitucional</strong><br>${info.note}<br>L’alleujament és temporal i només val mentre la pressió mantingui el mateix context.`});actions.appendChild(button)});box.appendChild(actions);
    const note=document.createElement('p');note.className='homeo-note';note.textContent='Homeòstasi no esborra pressió ni evita la mutació: només crea marge temporal, contextual i reversible.';box.appendChild(note);
    if((s.old.history||[]).length){const details=document.createElement('details');details.className='homeo-history';const summary=document.createElement('summary');summary.textContent=`Memòria homeostàtica · ${s.old.history.length} moviments`;details.appendChild(summary);s.old.history.slice().reverse().forEach(e=>{const row=document.createElement('div');row.className='homeo-history-row';row.innerHTML=`<strong>${e.label||e.mode}</strong><span>${e.mode==='reset'?'buffer alliberat':`−${Math.abs(Number(e.amount)||0)} pressió · base ${e.pressureBasis??e.rawPressure??'—'}`}</span>`;details.appendChild(row)});box.appendChild(details)}
  };
