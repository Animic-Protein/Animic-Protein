(() => {
  ...
  const germinate=entry=>{
    ...
    if(old)return old;
    const germ={id:`germen-${hash(entry.id)}`,sourceRelationId:entry.id,title:`Llavor · ${entry.title}`,desc:`Nascuda d’INTER NOS entre «${entry.a}» i «${entry.b}». ${entry.action||'Aquesta llavor encara busca la seva primera forma.'}`,aId:entry.aId,bId:entry.bId,createdAt:new Date().toISOString()};
  ...
  const render=()=>{
    ...
    readGerms().forEach((g,index)=>{
      const p=position(g,index),btn=document.createElement('button');
      btn.type='button';btn.className='germinated-node';btn.dataset.node=g.id;btn.dataset.title=g.title;btn.dataset.desc=g.desc;btn.style.left=`${p.left}%`;btn.style.top=`${p.top}%`;
      const mark=document.createElement('span');mark.className='germ-mark';mark.textContent='✦';
      const strong=document.createElement('strong');strong.textContent=g.title;
      const small=document.createElement('small');small.textContent='nascuda d’INTER NOS';
