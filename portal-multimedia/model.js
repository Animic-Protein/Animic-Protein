...
export const CODEX_MEDIA_SCHEMA_VERSION = "0.5.1";
...
// L'agent identifica qui o què ha aportat una font. No és autoria canònica ni decisió.
export function normalizeSourceAgent(agent){
  if(!agent)return null;
  if(typeof agent==='string')return{name:agent.trim()||'unknown',kind:'unknown'};
  const name=String(agent.name||agent.id||'unknown').trim()||'unknown';
  const kind=String(agent.kind||'unknown').trim().toLowerCase()||'unknown';
  return{id:agent.id||null,name,kind,external:agent.external??true};
}
...
export function reactivateLoopRecord(loopRecord,source={}){const at=now(),sourceId=source.id||makeId('src'),next=createCodexMediaRecord({source:{id:sourceId,kind:source.kind||loopRecord.source?.kind||'unknown',uri:source.uri??loopRecord.source?.uri??null,name:source.name||`${loopRecord.source?.name||'Loop'} · reactivat`,mime:source.mime||loopRecord.source?.mime||null,external:false,agent:source.agent??loopRecord.source?.agent??null,createdAt:at},provenance:{originId:loopRecord.provenance?.originId||loopRecord.source?.id||sourceId,parentId:loopRecord.id,rootRecordId:loopRecord.provenance?.rootRecordId||loopRecord.id,generation:(loopRecord.provenance?.generation||0)+1,createdBy:'archivum.loop-chamber.reactivate',reversible:true,history:[...(loopRecord.provenance?.history||[]),{at,action:'loop.reactivated',ref:loopRecord.loop?.id||loopRecord.id}]}});next.relation.push({id:makeId('rel'),at,kind:'reactivated-from-archivum-loop',target:loopRecord.id});return next}
