...
function installPhotoVideoBridge(){const photo=document.getElementById('photo');if(!photo||photo.dataset.videoBridge)return;photo.dataset.videoBridge='1';photo.addEventListener('change',event=>{const f=event.target.files?.[0];if(!f||!f.type.startsWith('video/'))return;requestAnimationFrame(()=>{const btn=document.querySelector('[data-open="videodrome"]');if(btn)btn.click();const status=document.getElementById('vstatus');if(status)status.textContent='Vídeo de Fototeca carregat · Videodrum preparat.'})},true)}
...
function boot(){installPhotoVideoBridge();const loops=document.getElementById('showLoops');if(loops&&!loops.dataset.lazyLoopTools){loops.dataset.lazyLoopTools='1';loops.addEventListener('click',()=>setTimeout(installLoopTools,0),{passive:true})}const idle=window.requestIdleCallback||((fn)=>setTimeout(fn,800));idle(normalizeVideodrumName)}
