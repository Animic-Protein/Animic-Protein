...
function applyBeta12(){
  ...
  const arch=$('#archivum');if(arch){
    const ey=arch.querySelector('.ey');if(ey)ey.textContent='ARCHIVUM / ARXIU VIU · MEMÒRIA GENERAL';
    const h=arch.querySelector('h2');if(h)h.textContent='Memòria que pot tornar a actuar';
    const p=arch.querySelector('.mut');if(p)p.textContent='Única memòria general del Portal. Conserva fonts, loops, transformacions, relacions i procedència. Els loops són una cambra interna d’Archivum, no un òrgan separat.';
    const btn=$('#showLoops');if(btn)btn.textContent='Cambra de loops';
    const loopEy=$('#looparium .ey');if(loopEy)loopEy.textContent='CAMBRA DE LOOPS · DINS ARCHIVUM';
    const loopH=$('#looparium h2');if(loopH)loopH.textContent='Loops conservats';
  }
  const loopTools=$('#looparium-tools');if(loopTools){const ey=loopTools.querySelector('.ey');if(ey)ey.textContent='CAMBRA DE LOOPS · ARCHIVUM';const title=loopTools.querySelector('.archive-title');if(title)title.textContent='Trobar i reactivar loops conservats.';}
