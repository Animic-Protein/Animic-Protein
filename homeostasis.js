(() => {
  ...
  const relatedIds=relations=>new Set(relations.flatMap(r=>[r?.aId,r?.bId,r?.sourceId,r?.targetId].filter(Boolean)));
  ...
  const snapshot=()=>{
    ...
    const linked=relatedIds(relations);
    const dormant=living.filter(g=>(Number(g.uses)||0)===0&&!linked.has(g.id));
    const prolonged=dormant.filter(g=>ageDays(g)>=7);
    const density=living.length?relations.length/living.length:0;
    let state='equilibri';
    if(living.length>=12&&density<.5)state='dispersió';
    else if(living.length>=10&&relations.length>=20)state='saturació';
    else if(prolonged.length>=4)state='latència';
    else if(compost.length>=2&&living.length<=2)state='regeneració';
    return {germs,seeds,relations,living,compost,dormant,prolonged,density,state};
  };

  const suggestion=s=>{
    if(s.state==='latència'&&s.prolonged.length){
      const candidate=s.prolonged.slice().sort((a,b)=>ageDays(b)-ageDays(a))[0];
      return {candidate,action:'consider-compost',text:`«${label(candidate)}» porta ${Math.floor(ageDays(candidate))} dies sense ús ni relacions. Potser convé escoltar-lo abans de decidir si torna al Compost.`};
    }
    if(s.state==='dispersió')return {action:'relate-first',text:'Hi ha molta matèria viva però poca densitat relacional. Abans de compostar, prova de relacionar o agrupar.'};
    if(s.state==='saturació')return {action:'pause',text:'Hi ha activitat i relacions abundants. No cal eliminar res automàticament: atura la sembra i observa què continua actiu.'};
  ...
  const render=()=>{
    ...
    box.innerHTML=`<p class="memory-title">Pols vital · Homeòstasi</p><p class="memory-empty">${guide.text}</p><div style="display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.4rem;margin-top:.65rem;font-size:.78rem"><span>${s.living.length}<br><small>vius</small></span><span>${s.compost.length}<br><small>compost</small></span><span>${s.seeds.length}<br><small>llavors</small></span><span>${s.relations.length}<br><small>relacions</small></span></div><p class="homeostasis-state" style="margin:.7rem 0 0;color:var(--metal)">Estat: ${s.state} · densitat ${s.density.toFixed(2)}</p>`;
    if(guide.candidate){
